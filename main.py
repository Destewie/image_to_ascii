...
if __name__ == "__main__":
    ...
    if len(sys.argv) not in [2, 3]:
        print(f"Usage: python3 main.py <image path> <optional: detail (min=1 ; max={len(PRECISIONS)})>")
        exit(1)
    ...
    precision = 4
    if len(sys.argv) == 3 and sys.argv[2].isdigit() and 1 <= int(sys.argv[2]) <= len(PRECISIONS):
